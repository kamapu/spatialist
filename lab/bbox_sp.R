...
#'
setMethod(
  "bbox_sp", signature(raster = "Extent"),
  function(raster, proj4 = "", geom = "point", ...) {
    ...
    if (geom == 1) {
      raster <- data.frame(
        fid = 1:4,
        longitude = c(
          raster@xmin, raster@xmin, raster@xmax,
          raster@xmax
        ),
        latitude = c(
          raster@ymin, raster@ymax, raster@ymax,
          raster@ymin
        )
      )
    ...
    if (geom == 2) {
      raster <- Polygons(
        list(Polygon(cbind(
          c(
            raster@xmin, raster@xmin,
            raster@xmax, raster@xmax
          ),
          c(
            raster@ymin, raster@ymax,
...
#'
setMethod(
  "bbox_sp", signature(raster = "RasterLayer"),
...
#'
setMethod(
  "bbox_sp", signature(raster = "RasterStack"),
  function(raster, geom = "point", ...) {
    return(bbox_sp(extent(raster), proj4string(raster), geom, ...))
  }
)
...
#'
setMethod(
  "bbox_sp", signature(raster = "RasterBrick"),
  function(raster, geom = "point", ...) {
    return(bbox_sp(extent(raster), proj4string(raster), geom, ...))
  }
)
